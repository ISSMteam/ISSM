...
void controltao_core(FemModel* femmodel){
	...
	/*Serialize*/
	IssmDouble* gradient=G->ToMPISerial();
	ControlSaveResults(femmodel, gradient);
...
int IssmMonitor(Tao tao, void *userCtx){
	...
	int         its,num_cost_functions;
	IssmDouble  f,gnorm,cnorm,xdiff;
	AppCtx2    *user      = (AppCtx2 *)userCtx;
	FemModel   *femmodel  = user->femmodel;
	int        *responses = NULL;
   IssmDouble *f_list    = NULL;

	femmodel->parameters->FindParam(&responses,&num_cost_functions,InversionCostFunctionsEnum);
   f_list = xNew<IssmDouble>(num_cost_functions);

	TaoGetSolutionStatus(tao, &its, &f, &gnorm, &cnorm, &xdiff, NULL);
	user->J[its]=f;

	/*Retrieve objective functions independently*/
	for(int i=0;i<num_cost_functions;i++) femmodel->Responsex(&f_list[i],EnumToStringx(responses[i]));

	/*Print stats*/
	if(its==0) InversionStatsHeader(num_cost_functions);
	InversionStatsIter(its+1, f, gnorm, f_list, num_cost_functions);
	xDelete<IssmDouble>(f_list);
