/*!\file MapOceanConnectivity
 * \brief: compute connectivity mask to ocean to detact lakes in the middle of grounded ice
 */

#include "./MapOceanConnectivityx.h"
#include "../../shared/shared.h"
#include "../../toolkits/toolkits.h"

#include "../InputUpdateFromVectorx/InputUpdateFromVectorx.h"

void MapOceanConnectivityx(FemModel* femmodel){
	...
	for(Object* & object : femmodel->elements->objects){
		...
		if(element->IsAllGrounded()){
			...
			if(element->IsFloating()){
				/* only look at element with no ice but that are floating (so pure ocean) */
	...
	/*OK, now put values of the mask in the ConnectedToOceanEnum */
	for(Object* & object : femmodel->elements->objects){
		Element* element = xDynamicCast<Element*>(object);
		int  numvertices = element->GetNumberOfVertices();
