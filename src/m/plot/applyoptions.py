    ...
    if options.getfieldvalue('colorbar', 1) == 1:
        ...
        if hasattr(ax, 'cax'): 
            ...
            if 1:
                ...
                cb_pos        =options.getfieldvalue('colorbarpos','right')
                if cb_pos in ['right','left']:
                    cb_orientation=options.getfieldvalue('orientation','vertical')
                elif cb_pos in ['top','bottom']:
                    cb_orientation=options.getfieldvalue('orientation','horizontal')

                fig=ax.get_figure()
                driver = make_axes_locatable(ax)
                cax    = driver.append_axes(cb_pos,
                                            size=options.getfieldvalue('colorbarsize','5%'),
                                            pad=options.getfieldvalue('colorbarpad',0.025))

                # Dummy colormap image
                ...
                # FIXME: When using subplots in matplotlib, this results in an oddly sized colorbar (especially, height does not match with corresponding axis.
                dummy = mpl.cm.ScalarMappable(cmap=cmap,norm=norm)
                dummy.set_array([])

                fig = ax.get_figure()
                cb= fig.colorbar(dummy, ax=ax, extend=extend, format=formatter,
                                   location=options.getfieldvalue('colorbarpos','right'),
                                   pad=options.getfieldvalue('colorbarpad',0.08),
                                   shrink=options.getfieldvalue('colorbarsize',1.0),
                                   aspect=options.getfieldvalue('colorbaraspect',20),
                                   )
        ...
        if options.exist('colorbartitle'):
            warnings.warn('Warning: The previous "colorbartitle" was applied as the colorbar label. It has now been moved to "colorbarYLabel" to match MATLAB\'s behavior.')
            cb.ax.set_title(options.getfieldvalue('colorbartitle'),fontsize=fontsize)
        if options.exist('colorbarYLabel'):
            if options.exist('colorbartitlepad'):
                ...
                cb.set_label(options.getfieldvalue('colorbarYLabel'), fontsize=fontsize)
        if options.exist('colorbartitle') & options.exist('colorbarYLabel'):
            warnings.warn('Warning: Are you sure you want to use both "colorbartitle" and "colorbarYLabel" at the same time?')
