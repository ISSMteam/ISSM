    ...
    if datasize[0] == numberofelements and datasize[1] == 1:
        ...
        if options.exist('mask'):
            ...
            if np.size(flags) == numberofvertices:
                # FIXME:
                # This change is only tested for numpy 2017+; check whether it works for older versions of numpy
                ...
                EltMask = np.asarray([np.any(tmp[i]) for i in range(numberofelements)])
