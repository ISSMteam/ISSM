	...
	methods
		...
		function self = cfsurfacesquare(varargin) % {{{
			if nargin==0,
				...
				self.surfaceid          = getfieldvalue(options,'surfaceid',1);
		...
		function md = checkconsistency(self,md,solution,analyses) % {{{
			...
			md = checkfield(md,'fieldname','self.surfaceid','field',self.surfaceid,'values',[1 2]);
		...
		function md = disp(self) % {{{
			...
			fielddisplay(self,'surfaceid','1: surface, 2: base');
		...
		function md = marshall(self,prefix,md,fid) % {{{
			...
			WriteData(fid,prefix,'data',self.surfaceid,'name','md.cfsurfacesquare.surfaceid','format','Integer');
