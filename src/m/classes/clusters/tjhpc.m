...
%      cluster=tjhpc();
%      cluster=tjhpc('np',3);
%      cluster=tjhpc('np',3,'login','username');
...
classdef tjhpc
	properties (SetAccess=public)
		...
		email         = 'END,FAIL';
		emailname  = '';
		email_domain  = 'tongji.edu.cn';
	end
	%}}}
	methods
		function cluster=tjhpc(varargin) % {{{

			%initialize cluster using default settings if provided
			if (exist('tjhpc_settings')==2), tjhpc_settings; end
		...
		function BuildKrigingQueueScript(cluster, md, filename) % {{{
         ...
         %checks
			...
			if ~isempty(cluster.email)
				...
				fprintf(fid,'#SBATCH --mail-user=%s@%s\n',cluster.emailname, cluster.email_domain);
			end
			fprintf(fid,'\n');
