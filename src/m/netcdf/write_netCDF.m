...
%   variable in every step is stored as a numeric or char variable.
	...
	if exist(filename, 'file') == 2
		delete(filename);
		if verbose, fprintf('Existing file %s deleted.\n', filename); end
	end

	ncid = netcdf.create(filename, 'NETCDF4');
	netcdf.putAtt(ncid, netcdf.getConstant('NC_GLOBAL'), 'history', ['Created by write_netCDF.m on ' datestr(now)]);
	...
	for fi = 1:numel(top_fields)
		...
		if isempty(val)
			if verbose, fprintf('  md.%s is empty – skipped\n', fname); end
			continue
		end
	...
	netcdf.close(ncid);
	if verbose, disp('Model successfully saved as NetCDF4.'); end
end
...
function write_value(val, vname, gid, root_ncid, path_str, verbose) % {{{
	...
	if iscell(val) && isempty(val)
		write_cell_empty(val, vname, gid, root_ncid, path_str, verbose);
		return
	end
	...
	% cell array of strings (e.g. requested_outputs)
	...
	error(['''' vname ''' is of type ''' class(val) ''' which is not supported yet']);
...
function write_cell_empty(val, vname, gid, root_ncid, path_str, verbose) % {{{
	assert(isempty(val))
	dim_name = 'char0';
	try
		dimid = netcdf.defDim(gid, dim_name, 0);
	...
	varid = netcdf.defVar(gid, vname, 'NC_CHAR', dimid);
	netcdf.putAtt(gid, varid, 'type_is', 'cell_empty');
end % }}}
