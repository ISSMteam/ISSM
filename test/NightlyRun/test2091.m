...
md.mesh=SlcMesh; %fixed mesh for platform-stable viscoelastic SLC regression
...
% Fixed north-pole cap load. The original BAMG circular-cap mesh was useful
% for benchmark plots, but made nightly archives platform-dependent.
pos1=find(md.mesh.lat>=80);
md.masstransport.spcthickness(:)=0;
md.masstransport.spcthickness(pos1)=1500*sqrt((cosd(90-md.mesh.lat(pos1))-cosd(10))./(1-cosd(10)));

%visco-elastic loading from love numbers that we load ourselves.
